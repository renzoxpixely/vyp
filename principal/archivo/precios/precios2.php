...
<table width="932" border="0" class="tabla2">
  <tr>
	...
	<td width="69"><div align="right"><strong><?php echo $quecosto?></strong></div></td>
	...
	<td width="65"><div align="center"><strong>MODIFICAR</strong></div></td>
	  ...
	  if ($val == 1)
	  {
	  $sql="SELECT codpro,desprod,pcostouni,costre,margene,prevta,preuni,factor,codmar,stopro,$Tabla FROM producto where desprod like '$search%' or codbar like '$search%' ";
	  }
	    if ($tipocosto == 1) //costo promedio
	    {
	    $sql="SELECT codpro,desprod,costpr/factor as pcostouni,costpr as costre,margene,prevta,preuni,factor,codmar,stopro,$Tabla FROM producto where desprod like '$search%'";
	    }
	  
	  if ($val == 2)
	  {
	  $sql="SELECT codpro,desprod,pcostouni,costre,margene,prevta,preuni,factor,codmar,stopro,$Tabla FROM producto where codmar = '$search'";
	    if ($tipocosto == 1) //costo promedio
	    {
	    $sql="SELECT codpro,desprod,costpr/factor as pcostouni,costpr as costre,margene,prevta,preuni,factor,codmar,stopro,$Tabla FROM producto where codmar = '$search'";
	    }
	  }
	  if ($val == 3)
	  {
	  $sql="SELECT codpro,desprod,pcostouni,costre,margene,prevta,preuni,factor,codmar,stopro,$Tabla FROM producto where $tabla > 0";
	    if ($tipocosto == 1) //costo promedio
	    {
	      $sql="SELECT codpro,desprod,costre/factor as pcostouni ,costpr as costre,margene,prevta,preuni,factor,codmar,stopro,$Tabla FROM producto where $tabla > 0";
	    }
	  }
	  $result = mysqli_query($conexion,$sql);
	  if (mysqli_num_rows($result)){
