...
class clsNotaDebito{
	...
	function insertarDetalleNotaDebito($idnd,$detalle){
		$sql = "INSERT INTO nota_debito_detalle(id,idnd, item, idproducto, cantidad, valor_unitario, precio_unitario, igv, porcentaje_igv, valor_total, importe_total)
	...
	function insertarNotaDebito($idemisor, $venta){
		$sql = "INSERT INTO nota_debito(id, idemisor, tipocomp, idserie, serie, correlativo, fecha_emision, codmoneda, op_gravadas, op_exoneradas, op_inafectas, igv, total, codcliente, tipocomp_ref, serie_ref, correlativo_ref, codmotivo)
	...
	function listarNotaDebito(){
		$sql = "SELECT * FROM nota_debito";
		global $cnx;
		return $cnx->query($sql);		
	}

	function obtenerUltimoComprobanteId(){
		$sql = "SELECT * FROM nota_debito ORDER BY id DESC LIMIT 1";
		global $cnx;
		return $cnx->query($sql);		
	}

	function obtenerComprobanteId($id){
		$sql = "SELECT * FROM nota_debito WHERE id=?";
		global $cnx;
		$pre = $cnx->prepare($sql);
