...
{
	public function EnviarComprobanteElectronico($emisor, $nombre, $rutacertificado, $ruta_archivo_xml, $ruta_archivo_cdr)
	{
		...
		if ($httpcode == 200) //200: La comunicacion fue satisfactoria
		{
			...
			if (isset($doc->getElementsByTagName('applicationResponse')->item(0)->nodeValue)) // si en la etique de rpta hay valor entra
			{
				...
				//print_r($response_array['data']) . "wwwwwwwwwwwwww<br>";
				// echo '$DocumentResponse  =' . $DocumentResponse['Description'] . "<br>";


				//echo $response_array['data']['Description'];
				return $response_array['data'];
				curl_close($ch);
				//echo 'Procesado correctamente, OK' . "<br>";
			} else {
				$estadofe = '2';
				$codigo = $doc->getElementsByTagName('faultcode')->item(0)->nodeValue;
				$mensaje = $doc->getElementsByTagName('faultstring')->item(0)->nodeValue;
				//LOG DE TRAX ERRORES DB
				echo 'Ocurrio un error con código: ' . $codigo . ' Msje:' . $mensaje . "<br>";
				...
				// echo 'posicion_coincidencia == ' . $posicion_coincidencia;
	...
	function ConsultarTicket($emisor, $cabecera, $ticket, $ruta_archivo_cdr)
	{
		// $ws = "https://e-beta.sunat.gob.pe/ol-ti-itcpfegem-beta/billService";
		$ws = 'https://e-factura.sunat.gob.pe/ol-ti-itcpfegem/billService';  //produccion
