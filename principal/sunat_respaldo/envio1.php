...
<link rel='STYLESHEET' type='text/css' href='../../css/calendar.css'>
 <table width="954" border="0">
    <tr>
      <td>
        <b><u>ENVIO DE FACTURAS Y BOLETAS A SUNAT</u></b>
        <br>
        ...
        <table cellpadding="3px" border="0px">
          <tr>
            ...
            <td><input type="button" value="Consultar" id="sunat-enviar" onclick="consultar_docs();"></td>
            <td colspan="2"><input type="button" value="Enviar en Masa" id="sunat-enviar-masa"></td>
            <td><input type="button" value="Limpiar Enviados" onclick="limpiar_enviados();" id="limpiar-enviados"></td>
            <td><input type="button" value="Comunicar Bajas" id="sunat-resumen"></td>
        ...
        <table width="99%" cellpadding="3px" border="1px" style="border-collapse:collapse;">
...
<script type="text/javascript">
  ...
  $('#sunat-enviar-masa').on('click', function(e){
    ...
    if(date.length > 0 && sucursal.length > 0){
      $.ajax({
        ...
        data: {action:'consultar-masa', date:date, sucursal:sucursal},
        dataType: 'JSON',
        beforeSend: function(){
  ...
  function consultar_docs(){
    ...
    if(date.length > 0 && sucursal.length > 0){
      $.ajax({
        type: 'POST',
        url: 'ajax/ajax_envio.php',
        data: {action:'consultar', date:date, sucursal:sucursal},
        dataType: 'JSON',
        beforeSend: function(){
          $(document.body).append('<span class="loading"><div></div></span>');
        },
  ...
  function limpiar_enviados(){
    ...
    if(date.length > 0 && sucursal.length > 0){
      $.ajax({
        type: 'POST',
        url: 'ajax/ajax_envio.php',
        data: {action:'limpiar_enviados', date:date, sucursal:sucursal},
        dataType: 'JSON',
        beforeSend: function(){
  ...
  function enviar_api_factura(id, json){
    var api = '<?=BASE_PATH?>/greenter/examples/factura.php';
    $.ajax({
      type: 'POST',
      url: api,
  ...
  function enviar_api_boleta(id, json){
    var api = '<?=BASE_PATH?>/greenter/examples/boleta.php';
    $.ajax({
      type: 'POST',
      url: api,
      data: {json:JSON.stringify(json)},
  ...
  function actualizar_documento(id, enviado, code, description, hash){
    $.ajax({
      type: 'POST',
      url: 'ajax/ajax_envio.php',
      data: {action:'actualizar', id:id, enviado:enviado, code:code, description:description, hash:hash},
      dataType: 'JSON',
      success: function(response){
        $('.btn-envio').removeAttr('disabled');
        $('#sunat-enviar').removeAttr('disabled');
    ...
    if(date.length > 0 && sucursal.length > 0){
      if(confirm('Esta apunto de generar una comunicacion de baja para enviar a SUNAT, 多Desea continuar?')){
        $.ajax({
          ...
          data: {action:'resumen', date:date, sucursal:sucursal},
          dataType: 'JSON',
          beforeSend: function(){
  ...
  function enviar_api_resumen(json, date, sucursal){
    var api = '<?=BASE_PATH?>/greenter/examples/comunicacion-baja.php';
    $.ajax({
      type: 'POST',
      url: api,
      data: {json:JSON.stringify(json)},
  ...
  function enviar_api_resumen_boletas(json, date, sucursal){
    var api = '<?=BASE_PATH?>/greenter/examples/resumen.php';
    $.ajax({
      type: 'POST',
      url: api,
      data: {json:JSON.stringify(json)},
  ...
  function guardar_resumen(enviado, code, description, hash, json, ticket, date, sucursal, type, final=false){
    $.ajax({
      type: 'POST',
      url: 'ajax/ajax_generar.php',
      data: {action:'guardar_resumen', enviado:enviado, code:code, description:description, hash:hash, ticket:ticket, type:type, json:JSON.stringify(json)},
      dataType: 'JSON',
      success: function(response){
