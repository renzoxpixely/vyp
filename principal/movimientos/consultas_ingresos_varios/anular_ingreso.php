...
if (mysqli_num_rows($result)){
	while ($row = mysqli_fetch_array($result)){
		...
		$date   = date('Y-m-d');
		$sqlInsert = "INSERT INTO kardex (codpro, nrodoc, tipmov, tipdoc, invnum, qtypro, fecha, sucursal, eliminado, factor, sactual) VALUES('$codpro', '$numdoc', '1', '5', '$cod', '$qtypro', '$date', '$sucursal', '1', '$factor', '$sactual')";
		error_log($sqlInsert);
