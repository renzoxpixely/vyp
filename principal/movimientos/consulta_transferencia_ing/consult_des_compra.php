...
$sql="SELECT numdoc,invfec,tipmov,tipdoc,usecod,sucursal, invnumrecib FROM movmae where invnum = '$cod'";
...
while ($row = mysqli_fetch_array($result)){
		...
		$date   = date('Y-m-d');
		$sqlInsert = "INSERT INTO kardex (codpro, nrodoc, tipmov, tipdoc, invnum, qtypro, fecha, sucursal, eliminado, factor, sactual) VALUES('$codpro', '$numdoc', '1', '1', '$cod', '$cant_unid', '$date', '$sucursal', '1', '$factor', '$sactual')";
		error_log($sqlInsert);
		mysqli_query($conexion, $sqlInsert)	;
	}
}
mysqli_query($conexion,"UPDATE movmae set val_habil = '1' where invnum = '$cod'");
mysqli_query($conexion,"UPDATE movmae set estado = '0' where invnum = '$invnumrecib'");
