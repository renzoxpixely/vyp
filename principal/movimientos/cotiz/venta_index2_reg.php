...
if ($cantidades <> 0)
{
	...
	$agot    = 1;
		$sql1="SELECT cuscod,invfec,sucursal FROM cotizacion where invnum = '$venta'";
		$result1 = mysqli_query($conexion,$sql1);
		if (mysqli_num_rows($result1)){
		while ($row1 = mysqli_fetch_array($result1)){
			$cuscod    = $row1['cuscod'];
	...
	if (($text1<> "") || ($text1<>0))
	{
		if ($numero == 0)
		{
			if ($cant_prod <> 0)
			{
				if ($agot == 1)
				{
			mysqli_query($conexion,"INSERT INTO cotizacion_det (invnum,invfec,cuscod,usecod,codpro,canpro,fraccion,factor,prisal,pripro,codmar,cospro,costpr) values ('$venta','$date','$cuscod','$usuario','$codpro','$text1','T','$factor','$pcunit','$pc','$codmar','$pcostouni','$costpr')");
				}
				else
				{
			mysqli_query($conexion,"INSERT INTO cotizacion_det (invnum,invfec,cuscod,usecod,codpro,canpro,fraccion,factor,prisal,pripro,codmar,cospro,costpr) values ('$venta','$date','$cuscod','$usuario','$codpro','$text1','T','$factor','$text2','$text3','$codmar','$pcostouni','$costpr')");	
				}
			}
		}
		else
		{
			if ($cant_prod <> 0)
			{
				...
				{
			mysqli_query($conexion,"INSERT INTO cotizacion_det (invnum,invfec,cuscod,usecod,codpro,canpro,fraccion,factor,prisal,pripro,codmar,cospro,costpr) values ('$venta','$date','$cuscod','$usuario','$codpro','$permitido','T','$factor','$pcunit','$pc','$codmar','$pcostouni','$costpr')");
				}
				else
				{
			mysqli_query($conexion,"INSERT INTO cotizacion_det (invnum,invfec,cuscod,usecod,codpro,canpro,fraccion,factor,prisal,pripro,codmar,cospro,costpr) values ('$venta','$date','$cuscod','$usuario','$codpro','$creal','F','$factor','$text2','$text3','$codmar','$pcostouni','$costpr')");	
				}
			}
		}
		if ($numero == 0)
